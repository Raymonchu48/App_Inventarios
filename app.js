...
function bindUI() {
  ...
  [els.searchInput, els.categoryFilter, els.stockFilter].forEach(el => el.addEventListener("input", renderProducts));
  els.btnNewProduct.addEventListener("click", openNewProductDialog);
  els.btnCloseDialog.addEventListener("click", () => els.productDialog.close());
...
function switchAuthTab(tab) {
  ...
  document.querySelectorAll(".auth-tab").forEach((panel, idx) => panel.classList.toggle("active", (tab === "login" && idx === 0) || (tab === "register" && idx === 1)));
}
...
function loadSavedConfig() {
  ...
  els.cfgPreviewUrl.textContent = cfg.url || "—";
}
function readConfig() {
...
async function loadProfile() {
  ...
  if (error) {
    state.profile = null;
    throwFriendly(error, "No pude cargar el perfil. Revisa schema.sql y el trigger de alta de usuarios.");
    return;
  }
  state.profile = data;
}
...
function applyPermissionsUI() {
  ...
  els.btnNewProduct.disabled = !canWrite || !active;
  els.btnSeed.disabled = !isAdmin() || !active;
  els.movementForm.querySelectorAll("input,select,textarea,button").forEach(el => el.disabled = !canWrite || !active);
  document.querySelectorAll(".requires-admin").forEach(el => el.classList.toggle("hidden", !isAdmin()));
  if (!active) {
...
function setConnectionState(kind, badgeText, text) {
  const klass = kind === "success" ? "success" : kind === "danger" ? "danger" : kind === "warning" ? "warning" : "neutral";
...
async function refreshAll() {
  ...
  await Promise.all([loadCategorias(), loadProductos(), loadMovimientos(), isAdmin() ? loadProfiles() : Promise.resolve()]);
...
async function loadProductos() {
  const { data, error } = await state.client.from("productos").select("*, categorias(id,nombre)").order("descripcion");
  if (error) return throwFriendly(error, "No pude cargar productos.");
  state.productos = data || [];
}
async function loadMovimientos() {
  const { data, error } = await state.client.from("movimientos").select("*, productos(id,descripcion,stock_code), profiles(full_name,email)").order("created_at", { ascending:false }).limit(120);
...
function renderCriticalList() {
  const rows = state.productos.filter(p => Number(p.cantidad || 0) <= Number(p.min_stock || 0)).sort((a,b)=>Number(a.cantidad||0)-Number(b.cantidad||0)).slice(0,10);
...
function renderRecentMoves() {
  ...
  state.movimientos.slice(0,8).forEach(m => {
    ...
    const who = m.profiles?.full_name || m.profiles?.email || 'usuario';
    const div = document.createElement("div");
    div.className = "item";
    div.innerHTML = `<strong>${escapeHtml(m.productos?.descripcion || 'Producto')} <span class="tag ${color}">${escapeHtml(m.tipo)}</span></strong><small>${formatNum(m.cantidad)} · ${formatDate(m.created_at)} · ${escapeHtml(who)}${m.nota ? ' · ' + escapeHtml(m.nota) : ''}</small>`;
...
function getFilteredProducts() {
  ...
  return state.productos.filter(p => {
    ...
    const matchStock = !stockMode || (stockMode === 'low' && qty <= min) || (stockMode === 'out' && qty <= 0) || (stockMode === 'ok' && qty > min);
...
function renderProducts() {
  ...
  els.productsTable.querySelectorAll("[data-action='edit']").forEach(btn => btn.addEventListener("click", () => openEditProductDialog(btn.dataset.id)));
...
function renderMovementHistory() {
  ...
  if (!state.movimientos.length) return renderEmpty(els.movementHistory, "No hay movimientos registrados.");
...
function renderUsersList() {
  ...
  els.usersList.querySelectorAll("[data-save-profile]").forEach(btn => btn.addEventListener("click", () => saveProfilePermissions(btn.dataset.saveProfile)));
}

async function saveProfilePermissions(id) {
  const role = els.usersList.querySelector(`[data-role-id='${id}']`).value;
  const is_active = els.usersList.querySelector(`[data-active-id='${id}']`).value === 'true';
...
function openNewProductDialog() {
  ...
  els.productId.value = "";
  els.productDialogTitle.textContent = "Nuevo producto";
...
function openEditProductDialog(id) {
  ...
  if (!p) return;
  els.productDialogTitle.textContent = "Editar producto";
  els.productId.value = p.id;
  els.pStockCode.value = p.stock_code || "";
  els.pDescripcion.value = p.descripcion || "";
  els.pPresentacion.value = p.presentacion || "";
  els.pStDate.value = p.st_date || "";
  els.pUnit.value = p.unit || "";
  els.pCantidad.value = p.cantidad ?? 0;
  els.pMinStock.value = p.min_stock ?? 0;
  els.pPagina.value = p.pagina ?? "";
  els.pCategoria.value = p.categoria_id || "";
  els.pCantidadOriginal.value = p.cantidad_original || "";
  els.pDetalleCantidad.value = p.detalle_cantidad || "";
  els.productDialog.showModal();
}

async function onSaveProduct(e) {
  e.preventDefault();
  if (!canEdit() || !isActiveUser()) return flash("No tienes permisos para editar productos.", true);
  const payload = {
  ...
  let query = state.client.from("productos");
  const { error } = els.productId.value
    ? await query.update(payload).eq("id", els.productId.value)
...
async function onSaveMovement(e) {
  e.preventDefault();
  if (!canEdit() || !isActiveUser()) return flash("Tu rol no puede registrar movimientos.", true);
...
function canEdit() { return ['admin','editor'].includes(state.profile?.role); }
function isActiveUser() { return state.profile?.is_active !== false; }
function cleanVal(v) { const s = (v ?? '').toString().trim(); return s || null; }
function formatNum(n) { return new Intl.NumberFormat('es-ES', { maximumFractionDigits: 2 }).format(Number(n || 0)); }
function formatDate(v) { return v ? new Date(v).toLocaleString('es-ES') : ''; }
